...
class Auth
{
    ...
    protected $_config = array(
        ...
        'AUTH_GROUP'        => 'bili_think_auth_group', // 用户组数据表名
        'AUTH_GROUP_ACCESS' => 'bili_think_auth_group_access', // 用户-用户组关系表
    ...
    public function getGroups($uid)
    {
        ...
        $sql = "select uid,group_id,title,rules from ".$this->_config['AUTH_GROUP_ACCESS']." a left join ".$this->_config['AUTH_GROUP']." g on a.group_id=g.id where a.uid=".$uid." and g.status='1'";
    ...
    protected function getAuthList($uid, $type)
    {
        ...
        //读取用户组所有权限规则
        //$rules = db($this->_config['AUTH_RULE'], [], false)->where($map)->field('name,condition')->select();
        $rules = DB::name($this->_config['AUTH_RULE'])->where($map)->field('name,condition')->select();
    ...
    protected function getUserInfo($uid)
    {
        ...
        if (!isset($userinfo[$uid])) {
            $map['id'] = $uid;
            $userinfo[$uid] = DB::name($this->_config['AUTH_USER'])->where($map)->find();
        }
        return $userinfo[$uid];
    }
